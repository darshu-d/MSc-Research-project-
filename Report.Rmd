...
Only variables measured at or before treatment initiation were included in the baseline table. Post-baseline outcome variables, including OS status and PFS status, were excluded from the baseline table because they are measured after treatment initiation and therefore should not be used to describe pre-treatment comparability across arms.
...
logrank_tab <- data.frame(
  ...
  `p-value` = round(c(p_os, p_pfs, p_comp), 3)
)
...
extract_effects <- function(model, outcome, model_type, estimate_name){
  broom::tidy(model, exponentiate = TRUE, conf.int = TRUE) %>%
    filter(term %in% c("ArmB", "ArmC")) %>%
    mutate(
      Outcome = outcome,
      Model = model_type,
    ...
    select(Outcome, Model, Treatment, Estimate, `p-value`)
}

make_wide_table <- function(df, estimate_label){
  df %>%
    mutate(
      Outcome = factor(Outcome, levels = c("OS", "PFS", "Composite")),
      Model = factor(Model, levels = c("Crude", "Adjusted"))
    ) %>%
    arrange(Outcome, Model, Treatment) %>%
    pivot_wider(
    ...
    transmute(
      Outcome,
      Model,
      `Arm B vs Arm A` = paste0(`Estimate_Arm B vs Arm A`, " (p = ", `p-value_Arm B vs Arm A`, ")"),
      `Arm C vs Arm A` = paste0(`Estimate_Arm C vs Arm A`, " (p = ", `p-value_Arm C vs Arm A`, ")")
    )
}

logistic_comparison_table <- bind_rows(
  extract_effects(log_os_arm, "OS", "Crude", "OR"),
  extract_effects(log_os, "OS", "Adjusted", "OR"),
  extract_effects(log_pfs_arm, "PFS", "Crude", "OR"),
  extract_effects(log_pfs, "PFS", "Adjusted", "OR"),
  extract_effects(log_comp_arm, "Composite", "Crude", "OR"),
  extract_effects(log_comp, "Composite", "Adjusted", "OR")
) %>%
  make_wide_table("OR")

cox_comparison_table <- bind_rows(
  extract_effects(cox_os_arm, "OS", "Crude", "HR"),
  extract_effects(cox_os, "OS", "Adjusted", "HR"),
  extract_effects(cox_pfs_arm, "PFS", "Crude", "HR"),
  extract_effects(cox_pfs, "PFS", "Adjusted", "HR"),
  extract_effects(cox_comp_arm, "Composite", "Crude", "HR"),
  extract_effects(cox_comp, "Composite", "Adjusted", "HR")
) %>%
  make_wide_table("HR")

kable(logistic_comparison_table,
      caption = "Crude and adjusted logistic regression treatment effects. Values are odds ratios with 95\\% confidence intervals and p-values.") %>%
  kable_styling(latex_options = c("hold_position"))

kable(cox_comparison_table,
      caption = "Crude and adjusted Cox proportional hazards treatment effects. Values are hazard ratios with 95\\% confidence intervals and p-values.") %>%
  kable_styling(latex_options = c("hold_position"))
```
...
Several alternative approaches can be used when proportional hazards assumptions are violated. Restricted mean survival time (RMST) summarises the expected event-free time up to a pre-specified time horizon and does not rely on the proportional hazards assumption [@royston2013rmst]. Weighted log-rank tests, such as Fleming--Harrington tests, allow different parts of the survival curve to be emphasised and may be useful when treatment effects occur earlier or later during follow-up [@harrington1982weighted; @fleming1991counting]. Landmark analysis can also be used to compare survival beyond a fixed time point, particularly when treatment effects or prognostic factors vary over time [@anderson1983landmark]. Time-varying coefficient Cox models are another option for non-proportional hazards, but they were not pursued here because this dataset did not contain repeated longitudinal measurements of covariates over follow-up, and the aim of the project was to compare standard endpoint, hierarchical, and multi-state approaches rather than to develop an extended Cox model [@grambsch1994].
...
Table 14 below presents the net-benefit results by treatment arm comparison. A positive global net benefit indicates that the first-named arm produced more overall clinical advantage than the comparator when OS and PFS contributions are combined.
...
nb_tab <- data.frame(
  ...
  `Overall p-value` = c(0.0310, 0.0499, NA),
  check.names = FALSE
)
...
The net-benefit analysis supported the same broad interpretation as the win-ratio analysis. Arm B showed a positive global net benefit of 0.220 compared with Arm A, with contributions from OS (0.134) and PFS (0.086). Arm C showed a positive global net benefit of 0.2015 compared with Arm A, with contributions from OS (0.1323) and PFS (0.0692). The comparison between Arms C and B showed no meaningful advantage, with a global net benefit of -0.049.

The p-values reported here (0.031 and 0.0499) apply only to the PFS component, not to the overall global net benefit, so they are not directly comparable to the win-ratio p-values in Section 3.7, which tested the combined OS and PFS result. No p-value was available for the global net benefit itself. The point estimates point in the same direction as the win-ratio findings, but without a global p-value, the overall advantage for Arms B and C cannot be called statistically significant.
...
```{r adjusted-msm-model, warning=FALSE, message=FALSE}
msm_cc_df <- msm_df %>%
  filter(!is.na(Sites), !is.na(LDH), !is.na(TMB), !is.na(JAK))
...
```{r adjusted-msm-table, results='asis'}
adjusted_msm_tab <- data.frame(
  Covariate = c(
    "Arm B vs Arm A",
    "Arm C vs Arm A",
    "Sites >= 3",
...
kable(adjusted_msm_tab,
      caption = "Adjusted multi-state transition hazard ratios for treatment arms and baseline covariates") %>%
  kable_styling(latex_options = c("hold_position"))
```
