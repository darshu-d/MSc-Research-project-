...
ggsurvplot(
  ...
  risk.table = FALSE, pval = TRUE, conf.int = TRUE,
  legend.title = "Arm", legend.labs = levels(clean_df$Arm)
)
```
...
msm_df <- clean_df %>%
  ...
  do({
    ...
    if (.$pfs_status == 1) {
      ...
      rows <- c(rows, list(
        data.frame(id = id, time = prog_time, state = 2, Arm = arm)
      ))
    }
    ...
    if (.$os_status == 1) {
      ...
      rows <- c(rows, list(
        data.frame(id = id, time = death_time, state = 3, Arm = arm)
      ))
      ...
      if (!(.$pfs_status == 1 && .$pfs_time == last_time)) {
        ...
        rows <- c(rows, list(
          data.frame(id = id, time = last_time, state = last_state, Arm = arm)
        ))
      }
    }
